...
var resources = (function () {
      ...
      if (!resources.phonecodes) {
        ...
        if (resources.phone.test(phoneNumber)) {
          for (var digit = 1; digit <= 3; digit++) {
            var countryCode = phoneNumber.substring(0, digit);
  ...
  resources.email = /^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)+$/;
...
var SignupService = (function () {
  ...
  SignupService.prototype.signup = function (info) {
    return __awaiter(this, void 0, void 0, function () {
      var errors, out, _i, errors_1, err, f, u, c, hashPassword, user, userId, ok, res;
      return __generator(this, function (_a) {
        switch (_a.label) {
          ...
          case 4:
            ...
            user = __assign(__assign({}, info), { password: hashPassword });
            userId = this.generateId();
            return [4, this.repository.save(userId, user)];
          case 5:
            ok = _a.sent();
            if (!ok) {
              return [2, this.status.error];
            }
            return [4, this.createConfirmCode(userId, user)];
          case 6:
            res = _a.sent();
...
function isEmail(email) {
  ...
  var regex = new RegExp(resources.email);
...
function isPassword(password) {
  var regex = new RegExp(resources.password);
...
exports.useUserRepository = useRepository;
...
var SqlRepository = (function () {
  ...
  SqlRepository.prototype.save = function (id, info) {
    ...
    if (!info.password || info.password.length === 0) {
      ...
      return this.db.execute(stmt.query, stmt.params).then(function (c) { return (c > 0 ? true : false); });
    }
    else {
      if (this.user === this.authen) {
        ...
        return this.db.executeBatch([stmt1, stmt2], true).then(function (c) { return (c > 0 ? true : false); });
      }
    }
  };
  ...
  SqlRepository.prototype.activate = function (id, password) {
    ...
    if (!password || password.length === 0) {
      ...
      if (this.user === this.authen) {
        ...
        return this.db.execute(query, params).then(function (c) { return (c > 0 ? true : false); });
      }
      else {
        p[this.id] = id;
        var p2 = p;
...
exports.UserRepository = SqlRepository;
exports.SqlUserRepository = SqlRepository;
function clone(obj) {
  var obj2 = {};
  var keys = Object.keys(obj);
